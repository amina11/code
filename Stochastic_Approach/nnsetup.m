function nn = nnsetup(architecture,lambda_2)
%NNSETUP creates a Feedforward Backpropagate Neural Network
% nn = nnsetup(architecture) returns an neural network structure with n=numel(architecture)
% layers, architecture being a n x 1 vector of layer sizes e.g. [784 100 10]

    nn.size   = architecture;     %betwork structure
    nn.n      = numel(nn.size);   %get layer number
    ...
    nn.dropoutFraction                  = lambda_2;            %  Dropout level (http://www.cs.toronto.edu/~hinton/absps/dropout.pdf)
    ...
    for i = 2 : nn.n   
        ...
        nn.W{i - 1}=-sqrt(6 / (nn.size(i - 1) + nn.size(i)))+2*sqrt(6 / (nn.size(i - 1) + nn.size(i)))*rand(nn.size(i), nn.size(i - 1)+1);
       %  nn.W{i - 1} = rand(nn.size(i), nn.size(i - 1)+1) - 0.5;
        ...
        %%for ADAM
        nn.mm{i - 1}=zeros(size(nn.W{i - 1}));
        nn.vv{i - 1}=zeros(size(nn.W{i - 1}));
    end
end
