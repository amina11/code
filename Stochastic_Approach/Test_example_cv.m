function   [test_misclassification]=Test_example_cv(data,output_path,total,lambda2)
...
%%
 %rand('state',0)
 nn = nnsetup([featureD  500 100  outputDim]);
 opts.numepochs =500;   %  Number of full s weeps through data
 opts.minibatchszie=20; 
 opts.batchnum=5;
 opts.plot = 0;
 [nn, L,loss] = nntrain(nn, train_x, train_y, opts, val_x, val_y,SS,total,lambda2);
